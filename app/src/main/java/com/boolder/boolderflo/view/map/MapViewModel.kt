...
import com.boolder.boolderflo.domain.CircuitProblemsRetriever
import com.boolder.boolderflo.domain.TopoDataAggregator
...
import com.boolder.boolderflo.domain.model.gradeRangeLevelDisplay
...
import com.boolder.boolderflo.view.detail.TopoView
import com.boolder.boolderflo.view.detail.VariantSelector
...
import com.boolder.boolderflo.view.ticklist.TickedProblemSaver
...
class MapViewModel(
    ...
    private val _screenStateFlow = MutableStateFlow(
        ScreenState(
            ...
            fFilterState = FFilterState(problemIds = emptyList()),
    ...
    override fun onFFilterChipClicked() {
        viewModelScope.launch {
            val fFilterIds = _screenStateFlow.value.fFilterState.problemIds
            val newFFilterIds = if (fFilterIds.isNotEmpty()) {
                emptyList()
            } else {
                problemRepository.problemIdsForFFilter()
            }

            _screenStateFlow.update {
                it.copy(fFilterState = FFilterState(problemIds = newFFilterIds))
            }
        }
    }
    ...
    data class FFilterState(val problemIds: List<Int>)
