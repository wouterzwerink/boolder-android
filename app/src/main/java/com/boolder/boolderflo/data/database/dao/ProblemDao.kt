...
import com.boolder.boolderflo.data.database.entity.ProblemWithAreaName
...
interface ProblemDao {
    ...
    @Query(
        ...
        SELECT id FROM problems
        WHERE sit_start = 1
        AND LOWER(COALESCE(grade, '')) IN ('6a', '6a+')
        AND LOWER(steepness) NOT IN ('slab', 'dyno')
        """
    )
    suspend fun problemIdsForFFilter(): List<Int>
}
